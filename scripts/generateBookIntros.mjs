...
const INTRO_DIR = path.join(process.cwd(), '성경 서론');
const OUTPUT_FILE = path.join(process.cwd(), 'src', 'data', 'bookIntrosData.ts');
...
const files = fs.readdirSync(INTRO_DIR);
const introsMap = {};

files.forEach((filename) => {
  if (!filename.endsWith('.md')) return;
  if (filename.includes('연대순')) return;
  ...
  if (bookId) {
    introsMap[bookId] = content.trim();
  }
});

// Fallback for Nehemiah if missing
if (!introsMap['neh']) {
  introsMap['neh'] = `# 느헤미야기 해설 및 개관
...
console.log(`Extracted intros for ${Object.keys(introsMap).length} books.`);

const outputCode = `// Auto-generated detailed Bible book introductions from Markdown files in '성경 서론' folder
export const BOOK_DETAILED_INTROS: Record<string, string> = ${JSON.stringify(introsMap, null, 2)};
`;
